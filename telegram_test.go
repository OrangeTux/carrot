...
func TestTelegram(t *testing.T) {
	...
	assert.Equal(t, telegram.ProtocolVersion, 4.2, "")
	assert.Equal(t, telegram.EquipmentId, 1234567890, "")
	assert.Equal(t, telegram.PowerUsedTariff1, 1.117, "")
	assert.Equal(t, telegram.PowerUsedTariff2, 4.491, "")
	assert.Equal(t, telegram.PowerProducedTariff1, 0.0, "")
	assert.Equal(t, telegram.PowerProducedTariff2, 0.0, "")
	assert.Equal(t, telegram.PowerCurrentTariff, 2, "")
	assert.Equal(t, telegram.PowerFailureCount, 1, "")
	assert.Equal(t, telegram.PowerFailureLongCount, 2, "")
	assert.Equal(t, telegram.VoltageSagsL1, 4, "")
	assert.Equal(t, telegram.VoltageSwellsL1, 3, "")
	assert.Equal(t, telegram.CurrentPowerUsage, 0.563, "")
	assert.Equal(t, telegram.CurrentPowerProduced, 0.0, "")
